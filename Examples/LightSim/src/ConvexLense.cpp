...
ConvexLense::ConvexLense(const std::string& name)
	: OpticalElement(name)
{
	m_painter = new LensePainter(this);
	m_diameter = 30;
	m_angle = M_PI / 4;
	m_lenseRadius = 30;
	...
	: OpticalElement(other)
{
	...
	m_lenseRadius = other.m_lenseRadius;
	m_pos = other.m_pos;
	m_n1 = other.m_n1;
	m_n2 = other.m_n2;
	addComponent(m_painter);
...
{
	return m_lenseRadius;
}

void ConvexLense::updateShape()
{
...
bool ConvexLense::getRaycastDistance(const QSFML::Utilities::Ray& ray, float& distanceOut) const
{
	...
	if (getLenseCollisionFactor(ray, false, fac, normal))
	{
		...
		circle->setPosition(ray.getPoint(fac));
...
/*
		...
		if (doesRefract)
		{
			...
			sf::Vector2f dir = QSFML::VectorMath::getRotatedUnitVector(refractAngle);
			
			refracted.setDirection(QSFML::VectorMath::getRotatedUnitVector(refractAngle));
...
bool ConvexLense::getLenseCollisionFactor(const QSFML::Utilities::Ray& ray, bool outgoingRay, float& smalestFactor, float& collisionNormal) const
{
	float fac1 = 99999999;
	float fac2 = 99999999;
	...
	bool c1 = getCircleElementCollisionFactor(ray, m_circlePos1, m_lenseRadius, angleRangeMin, angleRangeMax, fac1);
	/*if (outgoingRay)
	{
		angleRangeMin -= M_PI;
		angleRangeMax -= M_PI;
	...
	angleRangeMin -= M_PI;
	angleRangeMax -= M_PI;
	bool c2 = getCircleElementCollisionFactor(ray, m_circlePos2, m_lenseRadius, angleRangeMin, angleRangeMax, fac2);
...
bool ConvexLense::getCircleElementCollisionFactor(const QSFML::Utilities::Ray& ray, const sf::Vector2f& center,
	float radius, float minAngle, float maxAngle, float& smalestFactor) const
{
...
bool ConvexLense::getCircleCollisionFactor(const QSFML::Utilities::Ray& ray, const sf::Vector2f& center,
...
bool ConvexLense::getRaycastData(const QSFML::Utilities::Ray& ray,
...
void ConvexLense::getCollisionData(float alpha, float rayAngle, float& entryAngle, sf::Vector2f& point)
{
...
ConvexLense::LensePainter::LensePainter(ConvexLense* lense, const std::string& name)
	: Drawable(name)
	, m_lense(lense)
{
	m_resolution = 20;
}
ConvexLense::LensePainter::LensePainter(const LensePainter& other)
	: Drawable(other)
	, m_lense(nullptr)
{
	m_resolution = other.m_resolution;
}
COMPONENT_IMPL(ConvexLense::LensePainter);


void ConvexLense::LensePainter::draw(sf::RenderTarget& target,
