...
class ConcaveLense : public OpticalElement
{
	class LensePainter;
	friend LensePainter;
public:
	ConcaveLense(const std::string& name = "ConcaveLense");
	ConcaveLense(const ConcaveLense& other);
	~ConcaveLense();
	OBJECT_DECL(ConcaveLense);
